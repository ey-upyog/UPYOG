      ...
      if (type == actionTypes.SUBMIT_FORM_COMPLETE) {
        ...
        if (formKey.includes("MDMS")) {
          ...
          if (mdmsToast && mdmsToast.length) {
            dispatch((0, _actions3.toggleSnackbarAndSetText)(true, { labelName: mdmsToast, labelKey: mdmsToast }, "success"));
          }
        }
        // for login/register flow
        if (formKey === "otp") {
          var previousRoute = (0, _get2.default)(state.app, "previousRoute", "");
        ...
        if (formKey === "employeeOTP") {
          if (payload && payload.responseInfo && payload.responseInfo.status && payload.responseInfo.status == 200) dispatch((0, _actions3.toggleSnackbarAndSetText)(true, {
        ...
        if (formKey === "assignComplaint") {
          if (payload && payload.actionHistory && payload.actionHistory[0].actions[0].action === "assign") {
            dispatch((0, _actions3.setRoute)("/complaint-assigned/" + encodeURIComponent(payload.services[0].serviceRequestId)));
          } else if (payload && payload.actionHistory && payload.actionHistory[0].actions[0].action === "reassign") {
            dispatch((0, _actions3.setRoute)("/complaint-reassigned/" + encodeURIComponent(payload.services[0].serviceRequestId)));
          }
        }
        ...
        if (toast && toast.length) {
          dispatch((0, _actions3.toggleSnackbarAndSetText)(true, { labelName: toast, labelKey: toast }, "success"));
        }
      } else {
        next(action);
      }
